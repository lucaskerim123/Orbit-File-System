...
async function renderPdf(){ const pdfjsLib = await safeImport('/viewer-assets/pdfjs/pdf.mjs'); pdfjsLib.GlobalWorkerOptions.workerSrc = '/viewer-assets/pdfjs/pdf.worker.mjs'; viewer.innerHTML = '<div class="pdf-pages"></div>'; const pages = viewer.firstChild; const pdf = await pdfjsLib.getDocument({ url: rawUrl }).promise; setStatus(pdf.numPages + ' page' + (pdf.numPages === 1 ? '' : 's')); const maxWidth = Math.min(980, Math.max(320, viewer.clientWidth - 30)); for(let n=1;n<=pdf.numPages;n++){ const page = await pdf.getPage(n); const baseViewport = page.getViewport({ scale: 1 }); const scale = Math.min(2, maxWidth / baseViewport.width); const viewport = page.getViewport({ scale }); const canvas = document.createElement('canvas'); canvas.className = 'pdf-page'; canvas.width = Math.floor(viewport.width); canvas.height = Math.floor(viewport.height); canvas.style.width = Math.floor(viewport.width) + 'px'; canvas.style.height = Math.floor(viewport.height) + 'px'; pages.appendChild(canvas); await page.render({ canvasContext: canvas.getContext('2d'), viewport }).promise; } }
