...
import { startSorter,confirmSorter,buildFolderIndex,getSorterSettings,updateSorterSettings,resetSorterLearning } from './sorter-core.js';
...
function context(req){
  ...
  return {workspaceId:String(workspaceId),root:decodeURIComponent(String(root)),isAdmin:req.headers['x-sorter-admin']==='true',isOwner:req.headers['x-sorter-owner']==='true'};
}
...
async function api(req,res){
  ...
  if(req.method==='POST'&&url.pathname==='/api/startsorter'){ const run=await startSorter(ctx); const next={...run,lastRun:run.startedAt}; states.set(ctx.workspaceId,next); return send(res,200,next); }
